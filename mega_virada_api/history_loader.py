...
def fetch_history(force_update=False):
    ...
    try:
        ...
        # Try to save to cache, but don't fail if we can't (Render Read-Only issue)
        try:
            with open(CACHE_FILE, "w") as f:
                json.dump(data, f)
        except Exception as write_err:
            print(f"Warning: Could not write cache file: {write_err}")
...
def get_all_draws():
    ...
    return clean_drawsimport requests
import json
import os
from datetime import datetime
